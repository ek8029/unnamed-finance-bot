...
export function AuthShell({ subtitle, children, signupDescription = 'Create your account, then connect a brokerage or add your first position. No card required.' }: { subtitle: string; children: ReactNode; signupDescription?: string }) {
  ...
  return <div className="helm-auth">
    ...
    <main id="main-content" className="helm-auth-grid">
      ...
      <div className="helm-auth-form"><span className="helm-label">{signup ? 'START WITH HELM FREE' : 'YOUR INTELLIGENCE DESK'}</span><h1>{subtitle}</h1>{signup && <p className="helm-auth-subtitle">{signupDescription}</p>}<div className="helm-auth-fields">{children}</div><LegalFooter variant="minimal" /></div>
    </main>
  </div>;
}
