      ...
      <div className="space-y-4 text-left">
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <div className="flex-1 px-3 py-2 bg-[var(--color-bg-elevated)] border border-[var(--color-border-base)] rounded text-[13px] type-mono text-[var(--color-text-secondary)] truncate flex items-center">
              {referralLink}
            </div>
            <button
              onClick={() => copyToClipboard(referralLink)}
              className="px-3 py-2 bg-[var(--color-bg-elevated)] border border-[var(--color-border-base)] rounded hover:border-[var(--color-border-strong)] transition-colors flex items-center gap-1.5 text-[13px] text-[var(--color-text-secondary)] whitespace-nowrap"
            >
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 px-3 py-2 bg-[var(--color-bg-elevated)] border border-[var(--color-border-base)] rounded hover:border-[var(--color-border-strong)] transition-colors text-[13px] text-[var(--color-text-secondary)] flex items-center justify-center gap-1.5"
          >
      ...
      <form onSubmit={handleSubmit} className="space-y-2">
        <div className="flex flex-col sm:flex-row gap-2">
          <input
            ...
            className="flex-1 px-4 py-3 bg-[var(--color-bg-elevated)] border border-[var(--color-border-strong)] rounded text-[var(--color-text-primary)] placeholder-[var(--color-text-muted)] focus:outline-none focus:border-[var(--color-gold)] focus:ring-2 focus:ring-[var(--color-gold)]/30 transition-colors text-[15px]"
          />
          <button
            type="submit"
            disabled={status === 'loading'}
            className="px-6 py-3 bg-[var(--color-gold)] hover:bg-[var(--color-gold-hi)] text-[var(--color-bg-base)] font-semibold rounded transition-colors text-[15px] whitespace-nowrap disabled:opacity-50 flex items-center justify-center gap-2"
          >
        ...
        {!showReferral ? (
          ...
          <input
            ...
            className="w-full sm:w-48 px-4 py-3 bg-[var(--color-bg-elevated)] border border-[var(--color-border-base)] rounded text-[var(--color-text-primary)] placeholder-[var(--color-text-muted)] focus:outline-none focus:border-[var(--color-gold)] focus:ring-2 focus:ring-[var(--color-gold)]/30 transition-colors text-[15px] type-mono tracking-wider"
          />
        )}
      </form>
      {status === 'error' && (
        <p className="text-[var(--color-negative)] text-[13px] mt-2">{result.error}</p>
      )}
    </div>
  );
}
      ...
      <div className="space-y-2">
        <div className="flex flex-col sm:flex-row gap-2">
          <input
            ...
            disabled
            placeholder="you@example.com"
            className="flex-1 px-4 py-3 bg-[var(--color-bg-elevated)] border border-[var(--color-border-strong)] rounded text-[var(--color-text-primary)] placeholder-[var(--color-text-muted)] text-[15px]"
          />
          <button
            disabled
            className="px-6 py-3 bg-[var(--color-gold)] text-[var(--color-bg-base)] font-semibold rounded text-[15px] whitespace-nowrap flex items-center justify-center gap-2"
          >
            Join the waitlist
