...
export function PlatformSurface({ children }: { children: React.ReactNode }) {
  ...
  return <div className={excluded ? undefined : `helm-platform ${terminal ? 'helm-terminal' : auth ? 'helm-auth-route' : 'helm-public'}`} data-helm-page={path.split('/')[1] || 'home'}>{children}</div>;
}
