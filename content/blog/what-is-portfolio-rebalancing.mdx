...
seoTitle: "Portfolio Rebalancing: When and How to Do It"
...
faq:
  - q: "What is portfolio rebalancing?"
    a: "Rebalancing is returning a portfolio to its intended weights after market movement has pulled it away from them. A book set at 60 percent stocks and 40 percent bonds drifts as stocks outperform, and rebalancing sells enough of what grew and buys enough of what lagged to restore the target. It is a risk control step: without it, the portfolio steadily becomes more exposed to whatever has been winning."
  - q: "How does rebalancing a portfolio work?"
    a: "Compare each holding's current weight against its target, identify which are above and below their bands, then sell from the overweight and buy the underweight until the weights are back in range. Many people avoid selling entirely by directing new contributions and dividends into the underweight positions, which restores the balance over time without realizing any gains."
  - q: "How often should a portfolio be rebalanced?"
    a: "The two common approaches are calendar based and threshold based. Calendar rebalancing runs on a fixed schedule such as annually or semiannually. Threshold rebalancing acts only when an allocation drifts past a set band, often five percentage points from target. Threshold rules tend to trade less in calm markets and respond faster in volatile ones, and the two are frequently combined by checking on a schedule but acting only if a band is breached."
  - q: "Does rebalancing improve returns?"
    a: "The evidence is mixed and depends heavily on the period measured. In markets with strong sustained trends, rebalancing reduces returns because it trims the asset that keeps rising. Across choppier periods it can add a small amount by selling high and buying low. The consistent and better-documented effect is on risk: rebalancing keeps the portfolio near the risk level that was actually chosen."
  - q: "Does rebalancing a portfolio cost money?"
    a: "In a taxable account the main cost is tax, because selling an appreciated position realizes a capital gain in that year. Commissions are near zero at most brokers now, but bid-ask spreads and fund minimums still apply. In a 401(k) or an IRA there is no tax consequence to rebalancing, which is why it is often done inside those accounts first and the taxable account is adjusted with new contributions instead."
  - q: "How do you rebalance without triggering taxes?"
    a: "The usual approaches are directing new money and dividends into underweight positions instead of selling, doing the selling inside tax-advantaged accounts where sales are not taxable events, and pairing a required sale with a harvested loss elsewhere so the gain is offset. Selling specific high-basis lots rather than the broker's default order also reduces the gain recognized."
  - q: "How does Helm help with rebalancing?"
    a: "Helm shows current weights across every connected account on one view, with look-through into ETF holdings so the true weight of a name is visible rather than its weight in one fund. It flags when a position has drifted past a concentration threshold and reports the unrealized gain attached to trimming it. Helm reports the position; it does not place trades and does not tell you what to sell."
---
