...
seoTitle: "The Portfolio Concentration Problem"
...
published: true
faq:
  - q: "What is portfolio concentration risk?"
    a: "Concentration risk is the share of a portfolio's outcome that rests on one company, one sector or one underlying driver. It is measured by weight rather than by count, and by look-through weight rather than by the position list, because a name held directly and again inside several funds carries its combined weight, not the weight shown in any single line."
  - q: "How much of one stock is too much?"
    a: "There is no threshold in law or in accounting, and the honest answer depends on whether the rest of the portfolio and your income depend on the same thing. What can be stated precisely is the arithmetic: a position at 30 percent of a book falling 50 percent takes 15 percent off the total, and recovering that requires roughly 18 percent from everything else. Employer stock adds a second layer, because the salary and the position share one employer."
  - q: "How do I know if my portfolio is concentrated?"
    a: "Sum every route to each company, not just the direct holdings. Look up the top ten holdings of every fund you own and add those weights to any shares you hold outright. Investors who own an S&P 500 fund, a total market fund, a Nasdaq fund and a technology sector fund frequently find the same few names accounting for a far larger share of the total than any single line suggested."
  - q: "How do you reduce concentration without a large tax bill?"
    a: "The approaches that avoid realizing a large gain are directing all new contributions elsewhere until the weight falls on its own, selling gradually across tax years to stay inside a lower bracket, pairing a sale with a harvested loss from another position so the gain is offset, and donating appreciated shares to charity, which moves the position without realizing the gain. Which of these applies depends on the account type and the basis."
  - q: "Does owning index funds protect against concentration?"
    a: "Only partly. Cap-weighted index funds hold more of the largest companies by construction, so the top names in a broad US index account for a substantial share of it. Owning several such funds multiplies the overlap rather than diluting it. The measure that answers this is look-through weight per company across every fund and every account."
  - q: "How does Helm measure concentration?"
    a: "Helm reads connected accounts read-only through Plaid, maps ETF and index holdings back to the underlying companies, and reports single-name and sector weight on the combined book rather than per account. It also flags when separate holdings depend on the same driver, and it shows the dollar effect of a decline at the current weight. Concentration reporting is on the free tier."
---
