...
seoTitle: "Portfolio Diversification: What Works in 2026"
...
faq:
  - q: "What is portfolio diversification?"
    a: "Diversification is holding assets whose returns do not move together, so that a bad outcome in one does not arrive at the same moment as a bad outcome in the others. The measure that matters is correlation between holdings, not the count of them. Thirty positions that all rise and fall on the same driver are one bet written thirty times."
  - q: "How does diversification lower portfolio risk?"
    a: "It reduces the company-specific portion of risk, sometimes called unsystematic or idiosyncratic risk, which is the part attributable to one firm's own results. As holdings with low correlation are combined, those individual shocks partly cancel and the portfolio's volatility falls below the average volatility of its parts. What diversification cannot remove is market risk, the part that moves everything together in a broad selloff."
  - q: "Why is portfolio diversification important?"
    a: "Because concentration decides how a single adverse event lands. A portfolio resting on one company carries that company's accounting, its product cycle and its management in full. Spreading across firms, sectors and asset classes means no single failure is decisive, which is what allows a strategy to survive long enough for its expected return to show up."
  - q: "How many stocks make a portfolio diversified?"
    a: "Academic work on this generally finds most of the reduction in company-specific risk arrives in the first twenty to thirty names, with diminishing benefit after that. The count is the weaker half of the answer. Twenty-five holdings drawn from one sector remove far less risk than twelve drawn from uncorrelated parts of the market, and a single broad index fund can be more diversified than either."
  - q: "Can you be diversified and still be concentrated?"
    a: "Yes, and it is common. Cap-weighted index funds carry the same handful of large companies at the top, so an investor holding an S&P 500 fund, a total market fund, a Nasdaq fund and a technology sector fund owns four products with heavy overlap in the same names. The look-through exposure is concentrated even though the fund count suggests otherwise."
  - q: "Does diversification reduce returns?"
    a: "It narrows the distribution at both ends. Diversifying away from a single winning position removes the chance of that position's full upside and removes the chance of its full downside at the same time. What it targets is a better return for the risk taken rather than the highest possible return, which is why it is described as a risk decision rather than a return decision."
  - q: "How does Helm measure diversification?"
    a: "Helm reads connected accounts through Plaid and looks through ETF and index holdings to the underlying companies, so a name held directly and again inside three funds is counted once, at its true weight. It reports single-name and sector concentration on that combined view and flags when holdings you treat as separate rest on the same driver. Concentration reporting is on the free tier."
---
